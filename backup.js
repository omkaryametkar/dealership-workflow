...
app.post("/add-vehicle-allotment", (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query(checkSql, [customer_id, booking_id], (err, rows) => {

      if (err) return db.rollback(() => res.json({ status: "error" }));
      ...
      db.query(insertSql,
        ...
        (err) => {
          ...
          db.query(updateRefer, [customer_id, booking_id], (err) => {

            if (err) return db.rollback(() => res.json({ status: "error" }));

            // 4️⃣ CLOSE VEHICLE ALLOTMENT TAT
            const closeTat = `
              UPDATE department_tat
              SET end_time = NOW()
              WHERE booking_id = ? 
            ...
            db.query(closeTat, [booking_id], (err) => {

              if (err) return db.rollback(() => res.json({ status: "error" }));

              // 5️⃣ START INVOICE STAGE (ONLY IF NOT EXISTS)
              const insertTat = `
                INSERT INTO department_tat 
                (customer_id, booking_id, department, start_time)
                SELECT ?, ?, 'Invoice', NOW()
              ...
              db.query(insertTat, [customer_id, booking_id, booking_id], (err) => {

                if (err) return db.rollback(() => res.json({ status: "error" }));

                // 6️⃣ INSERT INTO INVOICE REFER (SAFE)
                const invoiceSql = `
                  INSERT INTO invoice_refer (customer_id, booking_id, status)
                  SELECT ?, ?, 'Pending'
                  WHERE NOT EXISTS (
